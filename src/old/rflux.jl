...
struct StateRouteFluxV2 <: AbstractStateRouteFlux
    "A map of input names (Symbol) and its variables (Num)"
    inputs::Vector{Num}
    "A map of output names (Symbol) and its variables (Num)"
    outputs::Vector{Num}
    ...
    function RiverRouteV2()
        @variables qin qout sriver
        @parameters lag
        exprs = (sriver + qin) / (1 + lag)
        return StateRouteFluxV2(qin, qout, [lag], sriver; exprs=exprs)
    end
end
