function StdMeanNormFlux(
    ...
    param_names::Vector{Vector{Symbol}}
)
    ...
    params = [[first(@parameters $nm = 0.0) for nm in names] for names in param_names]
...
function StdMeanNormFlux(
    ...
    SimpleFlux(fluxes, reduce(vcat, params), flux_exprs=flux_exprs)
end
...
function MinMaxNormFlux(
    ...
    SimpleFlux(fluxes, reduce(vcat, [p[2] for p in params]), flux_exprs=flux_exprs)
end
