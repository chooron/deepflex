...
uh_flux_1 = HydroModels.UnitHydroFlux(slowflow, slowflow_routed, x4, uhfunc=HydroModels.UHFunction(:UH_1_HALF), solvetype=:DISCRETE)
uh_flux_2 = HydroModels.UnitHydroFlux(fastflow, fastflow_routed, x4, uhfunc=HydroModels.UHFunction(:UH_2_FULL), solvetype=:DISCRETE)
...
config = (timeidx=timeidx, solver=solver)
result = gr4j_model(input, pas, config=config, convert_to_ntp=true);

slowflow_routed_vec = uh_flux_1(Matrix(reshape(result.slowflow,1,length(timeidx))), pas)
